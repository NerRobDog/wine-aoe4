...
struct aoe_fragment_key { uint8_t code[40], len, count, offsets[8], ends[8]; uint64_t targets[8]; };
...
{
 unsigned offset=0,steps;memset(k,0,sizeof(*k));
 for(steps=0;steps<16 && offset<available;steps++){
  ...
  if(h.opcode==0xe9||h.opcode==0xeb){
   if(h.p_66||h.p_seg||h.rex||k->count==8)return 0;
   ...
   k->code[offset]=0xe9;k->offsets[k->count]=offset+1;k->ends[k->count]=offset+5;k->targets[k->count++]=target;k->len=offset+5;return 1;
  }
  memcpy(k->code+offset,source+offset,h.len);
  if(h.opcode==0xc3||h.opcode==0xc2){if(h.p_66||h.p_seg||h.rex)return 0;k->len=offset+h.len;return 1;}
  ...
  if((h.flags&F_MODRM)&&h.modrm_mod==0&&h.modrm_rm==5){
   ...
   if(h.len<imm+4)return 0;disp=offset+h.len-imm-4;
   target=pc+offset+h.len+(int64_t)(int32_t)h.disp.disp32;
   memset(k->code+disp,0,4);k->offsets[k->count]=disp;k->ends[k->count]=offset+h.len;k->targets[k->count++]=target;
  }
  offset+=h.len;
 }
 return 0;
}
static int aoe_fragment_emit(const struct aoe_fragment_key *k,uint64_t pc,uint8_t output[40])
{
 unsigned i;memcpy(output,k->code,40);
