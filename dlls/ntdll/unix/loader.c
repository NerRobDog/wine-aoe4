...
#ifdef __x86_64__
extern NTSTATUS aoe_exception_context(void *args);
#endif
...
const unixlib_entry_t unix_call_wow64_funcs[] =
{
    ...
    wow64_pe_module_loaded, /* 64-bit private context operation is unsupported */
#endif
};
...
#if defined(__APPLE__) && defined(__x86_64__)
    if (getenv("AOELAB_SOFTFAULT_BRIDGE"))
    {
        const IMAGE_EXPORT_DIRECTORY *exports = get_module_data_dir(module, IMAGE_DIRECTORY_ENTRY_EXPORT, NULL);
        void *targets[2] = {(void *)find_named_export(module,exports,"__wine_aoe_softfault1"),(void *)find_named_export(module,exports,"__wine_aoe_softfault2")};
        void *bridge = (void *)0x180000000ULL;
        SIZE_T bridge_size = 4096;
        ULONG old_protect;
        unsigned int i;
        if (!targets[0] || !targets[1]) fatal_error("software fault entries missing\n");
        status = NtAllocateVirtualMemory(GetCurrentProcess(),&bridge,0,&bridge_size,MEM_RESERVE|MEM_COMMIT,PAGE_READWRITE);
        if (status || bridge != (void *)0x180000000ULL) fatal_error("cannot reserve software fault bridge (%x)\n",status);
        for (i=0;i<2;i++)
        {
            BYTE *entry=(BYTE *)bridge+16*i;
            entry[0]=0xff;entry[1]=0x25;
            memset(entry+2,0,4);memcpy(entry+6,&targets[i],8);
            entry[14]=entry[15]=0xcc;
        }
        status = NtProtectVirtualMemory(GetCurrentProcess(),&bridge,&bridge_size,PAGE_EXECUTE_READ,&old_protect);
        if (status) fatal_error("cannot protect software fault bridge (%x)\n",status);
    }
#endif
...
static void aoe_exec_sidecar(const char *sidecar, const char *loader, int argc, char **argv)
{
    ...
    /* The sidecar exec discards Wine preloader reservations. Re-enter the
     * normal preloader before server initialization, including Steam children. */
...
DECLSPEC_EXPORT void __wine_main( int argc, char *argv[] )
{
    ...
    init_paths();
#if defined(__APPLE__) && defined(__x86_64__)
    aoe_reexec_sidecar(argc,argv);
#endif
    if (!getenv( "WINELOADERNOEXEC" ) || argc <= 1) check_command_line( argc, argv );
    unsetenv( "WINELOADERNOEXEC" );
#ifdef __APPLE__
    /* Handshake is deferred until the PE runtime and bridge are mapped. */
#endif
