...
static void save_context( struct xcontext *xcontext, const ucontext_t *sigcontext )
{
    ...
    context->Rip    = aoe_cache_logical_pc(RIP_sig(sigcontext));
...
NTSTATUS signal_set_full_context( CONTEXT *context )
{
    ...
    if (!status && (context->ContextFlags & CONTEXT_CONTROL)==CONTEXT_CONTROL)
        get_syscall_frame()->rip=aoe_cache_target(context->Rip);
 ...
 */
/* Private software-exception context supplement. Use the same per-thread
 * debug register cache as save_context(), avoiding a server round trip. */
NTSTATUS aoe_exception_context(void *args)
{
    CONTEXT *context = args;
    struct amd64_thread_data *data = amd64_thread_data();
    context->SegCs = cs64_sel;
    context->SegSs = context->SegDs = context->SegEs = context->SegGs = ds64_sel;
    context->SegFs = data->fs;
    context->Dr0 = data->dr0; context->Dr1 = data->dr1;
    context->Dr2 = data->dr2; context->Dr3 = data->dr3;
    context->Dr6 = data->dr6; context->Dr7 = data->dr7;
    return STATUS_SUCCESS;
}
...
static void setup_raise_exception( ucontext_t *sigcontext, EXCEPTION_RECORD *rec, struct xcontext *xcontext )
{
    ...
    aoe_diag_add(AOE_SIGNAL_EXCEPTIONS_FORWARDED, 1);
    if (rec->ExceptionCode == EXCEPTION_ACCESS_VIOLATION) aoe_diag_add(AOE_FORWARDED_ACCESS_VIOLATIONS, 1);
    if (rec->ExceptionCode == EXCEPTION_ILLEGAL_INSTRUCTION) aoe_diag_add(AOE_FORWARDED_ILLEGAL_INSTRUCTIONS, 1);
...
static void setup_exception( ucontext_t *sigcontext, EXCEPTION_RECORD *rec )
{
    ...
    rec->ExceptionAddress = (void *)aoe_cache_logical_pc(RIP_sig(sigcontext));
...
static inline BOOL check_invalid_gsbase( ucontext_t *ucontext )
{
    ...
    aoe_diag_add(AOE_GSBASE_REPAIRS, 1);
...
{
    ucontext_t *ucontext = (aoe_diag_add(AOE_SEGV_SIGNALS, 1), init_handler( sigcontext ));
    EXCEPTION_RECORD rec = { 0 };
    struct xcontext context;

    rec.ExceptionAddress = (void *)aoe_cache_logical_pc(RIP_sig(ucontext));
    ...
    case TRAP_x86_PRIVINFLT:   /* Invalid opcode exception */
        aoe_diag_illegal(RIP_sig(ucontext));
#ifdef __APPLE__
        /* CW HACK 20186 */
    ...
    case TRAP_x86_PROTFLT:   /* General protection fault */
        aoe_diag_add(AOE_GENERAL_PROTECTION_TRAPS, 1);
        {
    ...
    case TRAP_x86_PAGEFLT:  /* Page fault */
        aoe_diag_add(AOE_PAGE_FAULTS, 1);
        switch ((ERROR_sig(ucontext) >> 1) & 0x09)
        {
        case EXCEPTION_READ_FAULT: aoe_diag_add(AOE_PAGE_READ_FAULTS, 1); break;
        case EXCEPTION_WRITE_FAULT: aoe_diag_add(AOE_PAGE_WRITE_FAULTS, 1); break;
        case EXCEPTION_EXECUTE_FAULT: aoe_diag_add(AOE_PAGE_EXECUTE_FAULTS, 1); break;
        }
        rec.NumberParameters = 2;
...
{
    ucontext_t *ucontext = (aoe_diag_add(AOE_TRAP_SIGNALS, 1), init_handler( sigcontext ));
    EXCEPTION_RECORD rec = { 0 };
    struct xcontext context;
...
{
    ucontext_t *ucontext = (aoe_diag_add(AOE_FPE_SIGNALS, 1), init_handler( sigcontext ));
    EXCEPTION_RECORD rec = { .ExceptionAddress = (void *)aoe_cache_logical_pc(RIP_sig(ucontext)) };
    struct xcontext context;
