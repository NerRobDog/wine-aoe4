...
/* This opt-in mapping contains aggregate counters only. Initialization runs
 * before signal handlers; hot paths only use lock-free atomic increments. */
unsigned long long *aoe_diag_counters;
static void __attribute__((constructor)) aoe_diag_init(void)
{
    const char *dir = getenv("AOELAB_COUNTERS_DIR");
    char name[64];
    unsigned long long *mapped;
    int directory, fd, attempt, saved_errno = errno;
    if (!dir || dir[0] != '/') return;
    directory = open(dir, O_RDONLY | O_DIRECTORY | O_CLOEXEC);
    if (directory < 0) { errno = saved_errno; return; }
    /* Wine can exec itself with the same PID. Each loaded instance gets a
     * separate mapping so a prior bootstrap file cannot disable collection. */
    for (attempt = 0; attempt < 32; attempt++)
    {
        snprintf(name, sizeof(name), "counters-%lu-%d.bin", (unsigned long)getpid(), attempt);
        fd = openat(directory, name, O_RDWR | O_CREAT | O_EXCL | O_CLOEXEC | O_NOFOLLOW, 0600);
        if (fd >= 0 || errno != EEXIST) break;
    }
    if (fd < 0) { close(directory); errno = saved_errno; return; }
    if (ftruncate(fd, 4096))
    {
        close(fd); unlinkat(directory, name, 0); close(directory); errno = saved_errno; return;
    }
    mapped = mmap(NULL, 4096, PROT_READ | PROT_WRITE, MAP_SHARED, fd, 0);
    close(fd);
    if (mapped == MAP_FAILED)
    {
        unlinkat(directory, name, 0); close(directory); errno = saved_errno; return;
    }
    close(directory);
    mapped[1] = 1;
    mapped[2] = getpid();
    mapped[3] = AOE_COUNTER_COUNT;
    aoe_diag_counters = mapped + 4;
    __atomic_store_n(&mapped[0], 0x414f454449414731ULL, __ATOMIC_RELEASE);
    errno = saved_errno;
}
                                        ...
                                        ULONG new_prot, ULONG *old_prot )
{
    ...
    aoe_diag_add(AOE_PROTECT_CALLS, 1);
    if (new_prot & 0xf0) aoe_diag_add(AOE_PROTECT_EXEC_REQUESTED, 1);
    ...
    if (status == STATUS_SUCCESS)
    {
        ...
        *size_ptr = size;
        *old_prot = old;
        aoe_diag_add(AOE_PROTECT_SUCCESS, 1);
        aoe_diag_add(AOE_PROTECT_BYTES, size);
        if ((old ^ new_prot) & 0xf0) aoe_diag_add(AOE_PROTECT_EXEC_CHANGED, 1);
    }
    else *old_prot = PAGE_NOACCESS;
    return status;
}
...
static void toggle_executable_pages_for_rosetta( HANDLE process, void *addr, SIZE_T size )
{
    ...
    if (!status && (info.AllocationProtect & 0xf0))
    {
        ...
        aoe_diag_add(AOE_ROSETTA_TOGGLE_BATCHES, 1);
                                      ...
                                      SIZE_T size, SIZE_T *bytes_written )
{
    ...
    aoe_diag_add(AOE_WRITE_CALLS, 1);
    ...
    aoe_diag_add(AOE_WRITE_BYTES, size);
...
{
    aoe_diag_add(AOE_FLUSH_CALLS, 1);
